...
void main(){
    ...
    float len=0.0;
    for(int i=0; i<100; i++){
        numerator= complexMultiplication(a*vec2(i,0), polynomial1(zi))+ complexMultiplication(b*vec2(i,0), polynomial2(zi))+ c;
        denominator= oneOverZ(complexMultiplication(polynomial2(zi), polynomial1(zi))+complexMultiplication(vec2(i,0), polynomial2(zi)));
        zi= complexMultiplication(numerator, denominator)+z0;
        if(length(zi)>500.){
            break;
    ...
    len= min(length(zi), 1.0);
    vec3 color= vec3(len);
    
    float phase = atan(zi.y, zi.x);
